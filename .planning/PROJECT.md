...
- [ ] Audio recording via PortAudio (cross-platform bindings)
...
- [ ] Audio feedback: embedded WAV chime on recording start/stop
- [ ] Configuration via TOML config file + environment variables
...
- [ ] Mic selection: system default, configurable via config
- [ ] Nix package: installable via Nix flake with NixOS module
...
|----------|-----------|---------|
| Go for implementation | Single binary, cross-compilation, strong CLI ecosystem | — Pending |
| PortAudio for audio capture | Cross-platform, mature, supports all backends | — Pending |
| Groq Whisper for transcription | Free tier, fast, accurate | — Pending |
| evdev for Linux hotkeys | Direct kernel input, no X11/Wayland dependency | — Pending |
| Embedded WAV assets | No external sound files, compiled into binary | — Pending |
| TOML config format | Readable, typed, standard for CLI tools | — Pending |
| XDG config location | Follows platform conventions | — Pending |
